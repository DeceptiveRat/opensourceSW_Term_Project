...
bool get_dns_query(const unsigned char *header_start, struct dns_query* dns_query_pointer)
{
	...
	unsigned char **domain_names = NULL;
	domain_names = (unsigned char**)malloc(sizeof(unsigned char**)*query_count);
	...
	else
	{
		...
		unsigned char** domain_names_additional = NULL;
		domain_names_additional = (unsigned char**)malloc(sizeof(unsigned char*)*additional_count);
	...
	while(byte != 0x00)
	{
		...
		if((byte & DNS_COMPRESSION_PTR) == DNS_COMPRESSION_PTR)
		{
			...
			*query_offset++;
			...
			unsigned char* temp = get_domain_name(query_start_pointer, &temp_offset);
			...
			unsigned char* name_pointer = (unsigned char*)malloc(sizeof(unsigned char)*domain_name_length);
